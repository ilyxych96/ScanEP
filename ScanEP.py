...
class scanEP(QtWidgets.QMainWindow):
    ...
    def zmt2xyz(self):  # not in use
        self.ui.statusbar.showMessage('Transformation in process')
        file0 = self.ui.lineEdit_11.text()
        try:
            functions.zmt2xyz(file0)
            self.ui.statusbar.showMessage('Transformation Done')
        except:
            self.ui.statusbar.showMessage('Something wrong')
